...
export function ConflictList() {
  ...
  return (
    <aside className="conflict-list" aria-label="Conflicts">
      <header className="panel-header">
        <span className="panel-title">Conflicts</span>
        ...
        {groups.map((group, i) => (
          <li key={group.id}>
            ...
            >
              ...
              <span className={`badge badge-${group.classification}`}>{group.classification}</span>
            </button>
          </li>
        ))}
        {groups.length === 0 && <li className="conflict-empty">No differences</li>}
      </ul>
    </aside>
  );
}
